...
public class Acceptor implements Runnable, Closeable {
	...
	public static Thread listen( int port, EventHandler eventHandler ) throws IOException {
		return listen( port, eventHandler, SimpleConnectionFactory.getInstance() );
	}

	public static Thread listen( int port, EventHandler eventHandler, ConnectionFactory connectionFactory ) throws IOException {
		String threadName = String.format( "%s::%d", Acceptor.class.getName(), Integer.valueOf( port ) );
		Thread thread = new Thread( new Acceptor( port, eventHandler, connectionFactory ), threadName );
		thread.start();
		return thread;
	}
